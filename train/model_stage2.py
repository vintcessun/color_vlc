"""
model_stage2.py — STN/TPS 亚像素对齐模型
==========================================
...
class LocalizationNet(nn.Module):
    ...
    输出：(B, 53, 2) 预测关键点坐标，范围 [-1,1]。
        ...
        采用“高分辨率积分回归（Integral Regression）”：
            features(1/32) -> deconv neck(上采样到1/8) -> heatmap head -> Spatial Soft-Argmax。
    ...
    def _spatial_softargmax(self, heatmaps: torch.Tensor) -> torch.Tensor:
        """
        对 heatmaps 执行二维 Spatial Soft-Argmax（积分回归）。

        Args:
            heatmaps: (B, K, H, W)
        Returns:
            coords:   (B, K, 2), 范围约在 [-1,1]
        """
        b, k, h, w = heatmaps.shape
        logits = heatmaps.view(b, k, h * w)

        # 数值稳定：先减去每个 heatmap 的最大值再 softmax
        logits = logits - logits.amax(dim=-1, keepdim=True)
        prob = F.softmax(logits, dim=-1)  # (B, K, H*W)

        ys = torch.linspace(
            -1.0, 1.0, steps=h, device=heatmaps.device, dtype=heatmaps.dtype
        )
        xs = torch.linspace(
            -1.0, 1.0, steps=w, device=heatmaps.device, dtype=heatmaps.dtype
        )
