"""
train_stage2_new.py — Stage 2 STN/TPS 亚像素对齐训练脚本（增强版）
===================================================================

数据源 : train/stage2_dataset/images/*.jpg   (任意分辨率失真 QR，训练时缩放到 OUT_SIZE)
         train/stage2_dataset/labels/*.npy   ((53,2) float32, [-1,1])
模型   : ColorQRStage2New (MobileNetV3-Small + 无参热力图上采样 + 温度 Soft-Argmax + TPS)
...
def _safe_torch_save(obj, path: str, retries: int = 3, retry_delay: float = 0.6):
    """
    原子保存并带重试，降低 Windows 上文件被瞬时占用或写入抖动导致的保存失败概率。
    """
    ...
    for i in range(retries):
        tmp_path = f"{abs_path}.tmp_{os.getpid()}_{int(time.time() * 1000)}_{i}.pt"
        try:
    ...
    raise RuntimeError(f"failed to save checkpoint: {abs_path}") from last_err
...
def _safe_savefig(
    ...
    """
    使用临时文件+原子替换保存图像，降低偶发文件占用导致的 OSError。
    返回是否保存成功；失败时不抛异常，避免中断训练主流程。
    """
    abs_path = os.path.abspath(path)
    os.makedirs(os.path.dirname(abs_path), exist_ok=True)

    for i in range(retries):
        tmp_path = f"{abs_path}.tmp_{os.getpid()}_{int(time.time() * 1000)}.png"
        try:
...
class TrainingLogger:
    ...
    def _plot(self):
        ...
        try:
            ...
            fig.tight_layout()
            if not _safe_savefig(fig, self.plot_path, dpi=100):
                print(f"[warn] failed to save plot: {self.plot_path}")
        finally:
            plt.close(fig)
        ...
        try:
            ...
            ax2.legend(loc="best")
            fig2.tight_layout()
            if not _safe_savefig(fig2, self.breakdown_plot_path, dpi=100):
                print(f"[warn] failed to save plot: {self.breakdown_plot_path}")
        finally:
            plt.close(fig2)
...
# 训练主函数
# ─────────────────────────────────────────────────────────────────────────────
def train_stage2_new(
    dataset_dir: str = "train/stage2_dataset",
    save_dir: str = "train/runs/stage2_stn_new",
    epochs: int = 1000,
    batch_size: int = 16,
    lr: float = 1e-4,
    num_workers: int = 0,
    vis_every: int = 100,  # 每隔多少 step 保存可视化
    save_every: int = 1,  # 每个 epoch 都保存 checkpoint
    device_str: str = "auto",
):
    # ── Device ───────────────────────────────────────────────────────────────
    if device_str == "auto":
    ...
    print(f"[Stage2-STN-NEW] device={device}  out_size={OUT_SIZE}")
    ...
    print(f"[Stage2-STN-NEW] {len(ds)} training samples")
    ...
    # ── Model ────────────────────────────────────────────────────────────────
    model = ColorQRStage2New(out_size=OUT_SIZE).to(device)
    print(
        f"[Stage2-STN-NEW] model params: "
    ...
    print(
        "[Stage2-STN-NEW] weighted loss enabled: "
        f"finder={FINDER_WEIGHT:.1f}, corners={CORNER_WEIGHT:.1f}, others=1.0"
    )
    print(
        "[Stage2-STN-NEW] strict loss enabled: "
    ...
    if ckpt_files:
        try:
            ...
            print(
                f"[Stage2-STN-NEW] resumed from {ckpt_files[-1]}, "
                f"start_epoch={start_epoch}  best_loss={best_loss:.6f}"
            )
        except RuntimeError as e:
            print(
                "[Stage2-STN-NEW] checkpoint incompatible with current setup, "
                f"starting fresh: {e}"
            )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (imgs, gt_kpts) in enumerate(loader):
            imgs = imgs.to(device)  # (B, 3, OUT_SIZE, OUT_SIZE)
    ...
    # ── Final export ─────────────────────────────────────────────────────────
    final_path = os.path.join(save_dir, "stage2_stn_new_final.pt")
    _safe_torch_save(model.state_dict(), final_path)
    print(f"[Stage2-STN-NEW] Training complete → {final_path}")
