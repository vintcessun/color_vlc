...
from model_stage2_new import CANON_KPTS_NORM, ColorQRStage2New, OUT_SIZE
...
def _find_stage2_model(model_path: Optional[str]) -> str:
    ...
    for run_name in run_names:
        ...
        final_new_cands = sorted(
            glob.glob(
                os.path.join(runs_dir, "**", run_name, "stage2_stn_new_final.pt"),
                recursive=True,
            )
        )
        if final_new_cands:
            return final_new_cands[-1]
    ...
    raise RuntimeError("stage2_new model not found under train/runs/stage2_stn_new")
...
def _load_stage2_weights(
    model: ColorQRStage2New, weight_path: str, device: torch.device
):
    ...
    try:
        ...
        raise RuntimeError(
            "checkpoint incompatible with current Stage2-New architecture. "
            "Please retrain with train/train_stage2_new.py, "
...
def run_test(
    ...
    print(f"[Test-Stage2-NEW] use model: {model_path}")
    print(f"[Test-Stage2-NEW] device: {device}")
    ...
    print("[Test-Stage2-NEW] done")
    print(f"[Test-Stage2-NEW] outputs: {out_dir}")
...
def main():
    ...
    parser.add_argument(
        "--out",
        default="train/inference_out/stage2_rectify_debug_new",
        help="output folder",
    )
